...
class ClaudeCodeContainerTester {
  ...
  async initialize() {
    ...
    await fs.mkdir(path.join('test', 'agent-communication', 'logs'), {
      recursive: true,
    });
...
async function main() {
  ...
  try {
    ...
    console.log(
      `   - Direct communication: ${directTestResults.length} events logged`,
    );
    console.log(
      `   - Protocol compliance: ${complianceResults.complianceScore}`,
    );
