...
async function processIssue(issueContext: IssueContext, githubToken: string): Promise<ContainerResponse> {
  ...
  try {
    ...
    try {
      ...
      try {
        await githubClient.createComment(
          ...
          [
            '⚠️ Claude Code failed in the container.',
            '',
    ...
    if (hasChanges) {
      ...
      await githubClient.createComment(parseInt(issueContext.issueNumber), `🔧 Created PR: ${pullRequest.html_url}`);
...
async function acpJsonRpcHandler(req: http.IncomingMessage, res: http.ServerResponse) {
  try {
    ...
    try {
      ...
      const handler = ACPHandlers[jsonRpcRequest.method as keyof typeof ACPHandlers];
