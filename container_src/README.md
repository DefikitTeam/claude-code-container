...
- `DAYTONA_WORKSPACE_ID` (optional) – when present the container assumes it is
  running in a persistent Daytona workspace. The workspace service stops
  treating directories as ephemeral, and the git service prefers `git pull` over
  re-cloning existing repositories.
- `WORKSPACE_ROOT` (optional) – overrides the directory used for persistent
  workspaces when `DAYTONA_WORKSPACE_ID` is set. The path is created if it does
  not already exist.
