...
This project optionally supports forwarding session events to an external Stream Broker for realtime UI subscriptions or auditing.

Environment variables (development example):

- `STREAM_BROKER_URL` - Base URL of the Stream Broker ingestion endpoint (e.g. http://localhost:3000). When undefined, stream posting is disabled and the container only writes notifications to stdout.
- `STREAM_BROKER_KEY` - API key used for authentication with the Stream Broker (used as `Authorization: Bearer <KEY>` or `X-Stream-Key`).
...
- The container will keep writing notifications to stdout as usual; enabling `STREAM_BROKER_URL` will additionally make the container attempt to POST events non-blockingly to the broker.
