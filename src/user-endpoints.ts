...
export function addUserEndpoints(app: Hono<{ Bindings: Env }>) {
  ...
  app.post("/register-user", async (c) => {
    try {
      ...
      if (!registrationRequest.installationId || !registrationRequest.anthropicApiKey) {
        return c.json({
          success: false,
      ...
      const apiKeyValidation = await validateAnthropicApiKey(registrationRequest.anthropicApiKey, true);
      ...
      return c.json({
        ...
        nextSteps: {
          step1: "Deploy your Cloudflare Worker with the provided userId and installationId",
 ...
 */
async function validateInstallationId(installationId: string): Promise<boolean> {
  try {
