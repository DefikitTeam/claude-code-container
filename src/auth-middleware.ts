 ...
 */
export async function containerAuthMiddleware(
  ...
  try {
    ...
    if (!authResult.success) {
      console.error(
        `Container auth failed for user ${userId}:`,
        authResult.error,
      );
 ...
 */
async function getUserConfigFromDO(
  userConfigDO: any,
  userId: string,
 ...
 */
export async function refreshAuthMiddleware(
  c: Context<{ Bindings: Env }>,
  next: Next,
) {
  try {
    ...
    if (authResult.success && authResult.auth) {
      ...
      if (expiresAt - now < fifteenMinutes) {
        console.log(
          `🔄 Refreshing auth for user ${authContext.userConfig.userId} (expires soon)`,
        );

        const refreshResult = await authContext.authManager.refreshAuth(
          authContext.userConfig,
        );
        if (!refreshResult.success) {
          console.error(
            `Failed to refresh auth for user ${authContext.userConfig.userId}`,
          );
          return c.json(
            {
        ...
        console.log(
          `✅ Auth refreshed for user ${authContext.userConfig.userId}`,
        );
      }
    }
 ...
 */
export async function quickAuthValidation(
  c: Context<{ Bindings: Env }>,
  next: Next,
) {
  try {
