...
async function processIssueHandler(req, res) {
    try {
        ...
        const repoFull = payload.repository?.full_name || process.env.REPOSITORY_NAME;
        const [owner, repo] = (repoFull || '').split('/');
        const githubToken = process.env.GITHUB_TOKEN || payload.config?.installationToken;
        const issue = payload.issue || { number: payload.issue_number, title: payload.issue_title, body: payload.issue_body };
        if (!owner || !repo)
        ...
        const cloneUrl = payload.repository?.clone_url || `https://github.com/${repoFull}.git`;
        ...
        logWithContext('PROMPT', 'Prepared prompt', { length: preparedPrompt.length });
        // For now we will not call Claude SDK here in the example; placeholder response
        const solution = { hasChanges: false, summary: 'Placeholder analysis - no changes applied.' };
        if (solution.hasChanges) {
            const branch = `claude-fix-${issue.number}`;
            // commit, push and create PR flow (left minimal here)
            await client.createPullRequest(`Fix: ${issue.title}`, solution.summary, branch, 'main');
            await cleanupWorkspace(workspace);
            return sendJson(res, 200, { success: true, message: 'PR created' });
        }
        else {
            await client.createComment(issue.number, `Automated analysis:\n\n${solution.summary}`);
            await cleanupWorkspace(workspace);
