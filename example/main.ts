  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const gitProcess = spawn(
        'git',
        ['clone', authenticatedUrl, workspaceDir],
        {
    ...
    logWithContext(
      'WORKSPACE',
      'Git repository cloned and configured successfully',
      {
...
async function initializeGitWorkspace(workspaceDir: string): Promise<void> {
  logWithContext('GIT_WORKSPACE', 'Configuring git workspace for development', {
    workspaceDir,
  });
...
async function createFeatureBranchCommitAndPush(
  workspaceDir: string,
  branchName: string,
  message: string,
): Promise<string> {
  logWithContext(
    'GIT_WORKSPACE',
    'Creating feature branch, committing, and pushing changes',
    {
      workspaceDir,
      branchName,
      message,
  ...
  try {
    ...
    logWithContext('GIT_WORKSPACE', 'Feature branch created and checked out', {
      branchName,
    });
    ...
    logWithContext('GIT_WORKSPACE', 'Branch pushed to remote successfully', {
      branchName,
    });
    ...
    logWithContext(
      'GIT_WORKSPACE',
      'Error creating branch, committing, or pushing changes',
      {
        error: (error as Error).message,
        branchName,
      },
...
async function readPRSummary(workspaceDir: string): Promise<string | null> {
  ...
  try {
    ...
    logWithContext(
      'GIT_WORKSPACE',
      'No PR summary file found or error reading',
      {
        summaryPath,
        error: (error as Error).message,
      },
...
async function processIssue(
  ...
  try {
    ...
    try {
      ...
      logWithContext(
        'CLAUDE_CODE',
        'Changed working directory for Claude Code execution',
        {
          originalCwd,
          newCwd: workspaceDir,
        },
      ...
      try {
        ...
        logWithContext('ISSUE_PROCESSOR', 'Change detection completed', {
          hasChanges,
        });
        ...
        if (hasChanges) {
          ...
          try {
            ...
            logWithContext(
              'ISSUE_PROCESSOR',
              'Failed to create pull request, posting comment instead',
              {
        ...
        logWithContext('CLAUDE_CODE', 'Restored original working directory', {
          originalCwd,
        });
      }
    } catch (outerError) {
      logWithContext(
        'ISSUE_PROCESSOR',
        'Error in Claude Code execution setup',
        {
...
async function processIssueHandler(
  ...
  if (requestBody) {
    try {
      ...
      logWithContext(
        'ISSUE_HANDLER',
        'Received issue context in request body',
        {
      ...
      if (issueContextFromRequest.ANTHROPIC_API_KEY) {
        process.env.ANTHROPIC_API_KEY =
          issueContextFromRequest.ANTHROPIC_API_KEY;
      }
      ...
      logWithContext(
        'ISSUE_HANDLER',
        'Environment variables updated from request',
        {
  ...
  const issueContext: IssueContext = {
    ...
    labels: process.env.ISSUE_LABELS
      ? JSON.parse(process.env.ISSUE_LABELS)
      : [],
